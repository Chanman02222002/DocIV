...
def client_handle_reschedule(call_id):
    ...
    if action == "accept":
        ...
        content = (
            ...
            f"{call.datetime.strftime('%Y-%m-%d %H:%M')} was accepted. {client_note}"
        )
    ...
    flash("Reschedule handled!", "success")
    return redirect(url_for('client_dashboard'))
...
# ============================================================
#           DOWNLOAD APPLICANTS FOR A GIVEN JOB (EXCEL)
# ============================================================
...
def download_job_applicants(job_id):
    ...
    for doc in doctors:
        rows.append({
            ...
            "Med Grad": doc.med_grad_month_year,
            "Residency": doc.residency,
            "Residency Grad": doc.residency_grad_month_year,
            "Fellowship(s)": doc.fellowship,
            "Fellowship Grad": doc.fellowship_grad_month_year,
            "Bachelors": doc.bachelors,
            "Bachelors Grad": doc.bachelors_grad_month_year,
            "MSN": doc.msn,
            "MSN Grad": doc.msn_grad_month_year,
            "DNP": doc.dnp,
            "DNP Grad": doc.dnp_grad_month_year,
            ...
            "Certification Area": doc.certification_specialty_area,
...
# ============================================================
#               ADMIN DOCTOR REGISTRATION (STAFF ONLY)
# ============================================================
...
def register_doctor():
    ...
    if form.validate_on_submit():
        ...
        existing_doc = Doctor.query.filter_by(email=form.email.data).first()

        if existing_doc:
            # Create login user and link to existing doctor
            new_user = User(
            ...
            existing_doc.user_id = new_user.id
            db.session.commit()
        ...
        # Link doctor → user
        new_user.doctor_id = new_doc.id
        db.session.commit()

        flash("New doctor account created!", "success")
        return redirect(url_for('admin_dashboard'))
...
# ============================================================
#               ADMIN CLIENT REGISTRATION (STAFF ONLY)
# ============================================================
...
# ============================================================
#              ADMIN VIEW DOCTOR PROFILES (OPTIONAL)
# ============================================================
...
def admin_view_doctor(doctor_id):
    ...
    return render_template("admin_doctor_profile.html", doctor=doctor, malpractice_cases=malpractice_cases)
# ============================================================
#                   SCHEDULING SYSTEM — CORE
# ============================================================

# This section handles:
# - Sending invites (client → doctor)
# - Accepting / declining invites (doctor)
# - Canceling meetings (doctor)
# - Requesting reschedule (doctor)
# - Client approval/denial of reschedule
# - Calendar event formatting


# ============================================================
#          DOCTOR: ACCEPT / DECLINE INVITE (PRIMARY ROUTE)
# ============================================================

@app.route('/handle_invite/<int:call_id>', methods=['POST'])
@login_required
def handle_invite(call_id):
    """Doctor accepts or declines a call invitation."""
    call = ScheduledCall.query.get_or_404(call_id)

    # Security check
    if current_user.role != "doctor" or current_user.doctor.id != call.doctor_id:
        flash("Unauthorized.", "danger")
        return redirect(url_for('doctor_dashboard'))
...
def doctor_handle_reschedule(call_id):
    """Doctor handles the result of client action about a reschedule."""
    call = ScheduledCall.query.get_or_404(call_id)
...
# ============================================================
#         CLIENT: APPROVE OR DENY RESCHEDULE (AUTHORITATIVE)
# ============================================================
